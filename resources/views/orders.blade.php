    ...
    <script>
        ...
        function getCustomerOrder() {
            $.ajax({
                type: 'get',
                url: "{{ url('') }}" +
                    '/api/client/customer/order?orderDetail=1&language_id=' + languageId +
                    '&productDetail=1&sortBy=id&sortType=DESC&currency=' + localStorage.getItem("currency"),
                headers: {
                ...
                success: function(data) {
                    if (data.status == 'Success') {
                        ...
                        for (i = 0; i < data.data.length; i++) {
                            ...
                            if (data.data[i].currency != null && data.data[i].currency != 'null' && data.data[i]
                                .currency != '') {
                            ...
                            clone.querySelector(".order-product-name").setAttribute('href', '/orders/' + data
                                .data[i].order_id);

                            clone.querySelector(".order-status").innerHTML = data.data[i].order_status ===
                                'Pending' ? data.data[i].order_status + '/ <button href="/orders/' + data.data[
                                    i].order_id + '" onClick="cancelStatus(' + data.data[i].order_id +
                                ')" style="font-size:10px" class="btn p-2  btn-danger">Cancel Order</button>' :
                                data.data[i].order_status;

                            clone.querySelector(".order-detail").innerHTML = '<a href="/orders/' + data.data[i]
                                .order_id + '">View Detail</a>';

                            if (data.data[i].order_detail != null && data.data[i].order_detail != 'null' && data
                                .data[i].order_detail != '') {
                                if (data.data[i].order_detail[0].product != null && data.data[i].order_detail[0]
                                    .product != 'null' && data.data[i].order_detail[0].product != '') {
                                    if (data.data[i].order_detail[0].product.product_type == 'variable') {
                                        if (data.data[i].order_detail[0].product_combination.gallary != null) {
                                            ...
                                            for (loop = 0; loop < data.data[i].order_detail[0]
                                                .product_combination.combination
                                                .length; loop++) {
                                                if (data.data[i].order_detail[0].product_combination.combination
                                                    .length - 1 == loop) {
                                                    name += data.data[i].order_detail[0].product_combination
                                                        .combination[loop].variation
                                                        .detail[0].name;
                                                } else {
                                                    name += data.data[i].order_detail[0].product_combination
                                                        .combination[loop].variation
                                        ...
                                        if (data.data[i].order_detail[0].product.detail != null) {
                                            ...
                                            clone.querySelector(".order-product-name").innerHTML = data.data[i]
                                                .order_detail[0].product.detail[0].title;
                                        }
                                    }
                                }
                            }
